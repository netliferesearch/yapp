        ...
        .then(() => {
          ...
          if(user !== null) {
            ...
            .then(() => {
              ...
              database.ref(`unregisteredCards/${cardId}`)
              .remove()
              .then(() => {
                // Add user to users node with cardId as key, uid and nickname.
                database.ref(`users/${cardId}`)
                .set({
                  uid: user.uid,
                  nickname: nickname
                })
                ...
                .catch(() => {
                  dispatch({
                    type: actionType.doSignupRejected,
                    error: errorMsg('signup', null, 'User is created but we could not update users node with userdata.')
                  });
                })
              });
            })
