...
export default function readSponsors() {
  const query = `
        *[_type == 'konferanse' && _id == "1dac660d-d163-4c92-9a3f-b96c2a3dd0c7"][0]{
          ...
          "superPartners": sponsors[level == "superpartner"]{_key,level,"sponsor": ref->{..., "image": image.asset->url}},
          "mainPartners": sponsors[level == "mainpartner"]{_key,level,"sponsor": ref->{..., "image": image.asset->url}},
          "partners": sponsors[level == "partner"]{_key,level,"sponsor": ref->{..., "image": image.asset->url}},
        }`;
  return dispatch => {
    sanity
      .fetch(query)
